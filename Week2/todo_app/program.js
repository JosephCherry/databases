...
class TodoModel {
    ...
    markCompleted(todoItemId, callback) {
        ...
        dbConnection.query(updateIsCompleted, todoItemId, function (err, results) { 
            if (err) {
                callback(err);
                return;
            }
...
dbConnection.connect(function (err) {
    ...
    todoModel.create({ text: 'go to conference', user_id: 4 }, function (err) {
        if (err) {
    ...
    todoModel.update([48, 'prepare for class discussions'], function (err, res) {
        if (err) {
    ...
    todoModel.delete(49, function(err, res){
        if (err){
    ...
    todoModel.tagTodoItem({todo_item_id:51, tag_id:5,}, function(err,res){
        if (err){
